...
def new_nms(detections, nms_iou_thershold=0.4, prob_filter=0.3):
    print('detections',np.shape(detections))
    filter_detection= []
    final_box=[]
    for det in range(detections.shape[0]):
        if detections[det, 4] >= prob_filter:
            ...
            pass
    filter_detection= sorted(filter_detection, key= lambda x : x[4], reverse= True)
    boxes_after_nms= []
    ...
    while boxes_after_nms:
        ...
        box_after_nms= [
            ...
            or abs(box[8] - post_nms_chosen_box[8]) >0.5
        ]
